    ...
    <div className="space-y-8 pb-12">
      <section className="glass-card relative overflow-hidden rounded-3xl border border-slate-800 p-8 shadow-2xl sm:p-10">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-8 lg:flex-row lg:items-center">
          <div className="space-y-3">
            <span className="inline-flex items-center rounded-full border border-emerald-500/30 bg-emerald-500/15 px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-emerald-300"><CheckCircle2 className="mr-1.5 h-4 w-4 text-emerald-400" />{t("student.activeCandidate", "Verified academic examiner")}</span>
            <h1 className="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">{t("student.welcome", "Welcome back")}, {user?.fullName || "Examiner"}!</h1>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/examiner/questions/create" className="inline-flex items-center justify-center gap-2.5 rounded-2xl bg-indigo-600 px-6 py-4 text-sm font-bold text-white shadow-xl shadow-indigo-600/25 transition-all hover:scale-[1.02] hover:bg-indigo-500"><PlusCircle className="h-5 w-5" />{t("examiner.createAssessment", "Create Question")}</Link>
            <Link href="/examiner/exams?create=1" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-700 bg-slate-900 px-5 py-4 text-sm font-semibold text-slate-300 transition-colors hover:bg-slate-800 hover:text-white"><FileText className="h-4 w-4" />{t("examiner.createAssessment", "Create Assessment")}</Link>
          </div>
        </div>
      </section>

      <section aria-label="Live assessment metrics" className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard title={t("examiner.questionBank", "Question Bank")} value={metricValue(metrics.questions)} subtitle="Questions you have created" icon={BookOpen} color="indigo" />
        <StatCard title={t("student.liveAssessments", "Active Assessments")} value={metricValue(metrics.activeExams)} subtitle="Currently accepting attempts" icon={FileText} color="indigo" />
        <StatCard title={t("examiner.published", "Published Assessments")} value={metricValue(metrics.publishedExams)} subtitle="Available or scheduled" icon={FolderKanban} color="cyan" />
        <StatCard title={t("examiner.candidates", "Candidate Attempts")} value={metricValue(metrics.candidateAttempts)} subtitle="Recorded across your assessments" icon={Users} color="emerald" />
      </section>
      ...
      <section className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="glass-card group flex flex-col justify-between rounded-3xl border border-slate-800 p-7 shadow-xl transition-all hover:border-indigo-500/40">
          <div className="space-y-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-indigo-500/30 bg-indigo-600/20 text-indigo-400 transition-transform group-hover:scale-110"><PlusCircle className="h-6 w-6" /></div><h2 className="text-xl font-bold text-white">{t("examiner.createAssessment", "Create assessment content")}</h2><p className="text-sm leading-relaxed text-slate-400">Create questions, then build an assessment with sections, timing, and marking rules.</p></div>
          <div className="pt-6"><Link href="/examiner/questions/create" className="inline-flex items-center gap-2 text-sm font-bold text-indigo-400 hover:text-indigo-300">{t("examiner.createAssessment", "Create a question")}<ArrowRight className="h-4 w-4" /></Link></div>
        </div>
        <div className="glass-card group flex flex-col justify-between rounded-3xl border border-slate-800 p-7 shadow-xl transition-all hover:border-violet-500/40">
          <div className="space-y-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-violet-500/30 bg-violet-600/20 text-violet-400 transition-transform group-hover:scale-110"><FolderKanban className="h-6 w-6" /></div><h2 className="text-xl font-bold text-white">{t("examiner.manageAssessments", "Manage assessments")}</h2><p className="text-sm leading-relaxed text-slate-400">Open an assessment to manage questions, review attempts, grade answers, and view analytics.</p></div>
          <div className="pt-6"><Link href="/examiner/exams" className="inline-flex items-center gap-2 text-sm font-bold text-violet-400 hover:text-violet-300">{t("examiner.manageAssessments", "Open assessments")}<ArrowRight className="h-4 w-4" /></Link></div>
        </div>
      </section>
    </div>
  );
